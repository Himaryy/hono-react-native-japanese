...
- 🔴 `Backend/` — 0 files, building from scratch
- 🔴 `Frontend/` — 0 files, building from scratch
- 🔴 Neon DB — not yet created
- 🔴 Vercel — not yet deployed
- 🔴 Static content JSON — not yet created
...
### `/karpathy-guidelines`
Invoke before writing, reviewing, or refactoring code. Prevents overcomplication, enforces surgical changes, surfaces hidden assumptions, defines verifiable success criteria.

### `/impeccable`
Use for all UI work — screens, components, styling. Produces production-grade React Native UI. Trigger before building any screen or component.

### Caveman mode + Cavecrew
Token efficiency tool. Active via session hook. Use `full` (default) or `ultra`. Commands: `/caveman full`, `/caveman ultra`, `stop caveman`.
Cavecrew subagents — use whenever applicable to save main context:
- `cavecrew-investigator` — locate code, map files, find where X is defined
...
Check here before writing custom UI primitives. Pairs with NativeWind.
...
Rules:
- For codebase questions, first run `graphify query "<question>"` when graphify-out/graph.json exists. Use `graphify path "<A>" "<B>"` for relationships and `graphify explain "<concept>"` for focused concepts. Return scoped subgraph — smaller than GRAPH_REPORT.md or raw grep.
